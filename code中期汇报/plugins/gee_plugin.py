...
def get_gee_stats(roi_geometry, start_date, end_date, output_dir, log_callback=None):
    ...
    viirs_col = ee.ImageCollection("NOAA/VIIRS/DNB/MONTHLY_V1/VCMSLCFG") \
        ...
        .select('avg_rad')
    reducer = ee.Reducer.mean().combine(ee.Reducer.sampleStdDev(), sharedInputs=True) \
        .combine(ee.Reducer.minMax(), sharedInputs=True) \
        .combine(ee.Reducer.median(), sharedInputs=True)
    def calc_viirs(img):
        stats = img.reduceRegion(reducer=reducer, geometry=roi_geometry, scale=500, maxPixels=1e9)
    ...
    try:
        ...
        if viirs_data:
            ...
            exist_cols = [c for c in ['Date'] + list(col_mapping.keys()) if c in viirs_df.columns]
            if exist_cols:
                viirs_df = viirs_df[exist_cols]
                rename = {k: v for k, v in col_mapping.items() if k in viirs_df.columns}
    ...
    def calc_ndvi(img):
        mean = img.reduceRegion(reducer=ee.Reducer.mean(), geometry=roi_geometry, scale=10, maxPixels=1e9)
        return ee.Feature(None, mean).set({'Date': img.date().format('YYYY-MM-dd')})
    try:
        ndvi_feats = s2_ndvi.map(calc_ndvi).getInfo()
        ndvi_data = [f['properties'] for f in ndvi_feats['features'] if f['properties'].get('NDVI') is not None]
        if ndvi_data:
    ...
    lst_reducer = ee.Reducer.mean().combine(ee.Reducer.minMax(), sharedInputs=True).combine(ee.Reducer.median(), sharedInputs=True)
    def calc_lst(img):
        stats = img.reduceRegion(reducer=lst_reducer, geometry=roi_geometry, scale=30, maxPixels=1e9)
        return ee.Feature(None, stats).set({'Date': img.date().format('YYYY-MM-dd')})
    try:
        lst_feats = l8_lst.map(calc_lst).getInfo()
        lst_data = [f['properties'] for f in lst_feats['features'] if f['properties'].get('LST_Celsius_mean') is not None]
        if lst_data:
            lst_df = pd.DataFrame(lst_data)
            expected = ['Date', 'LST_Celsius_mean', 'LST_Celsius_min', 'LST_Celsius_max', 'LST_Celsius_median']
